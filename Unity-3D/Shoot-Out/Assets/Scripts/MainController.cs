...
public class MainController : MonoBehaviour
{
	...
	public Color[] TotalColor = {Color.white,Color.magenta,Color.black,Color.blue,Color.cyan,Color.green,Color.grey,Color.red,Color.yellow };
    ...
    void emissionDisks()
    {
        ...
        {
            diskIds.Add(UFOFactory.getInstance().getDiskId());
            disks.Add(UFOFactory.getInstance().getDiskObject(diskIds[i]));
            ...
            disks[i].GetComponent<Rigidbody>().AddForce(emissionDirection * Random.Range(emissionSpeed * 5, emissionSpeed * 15) / 10, ForceMode.Impulse);
        }
        if(scene.getTrial() == 5)
        {
            scene.nextRound();
    ...
    {
        UFOFactory.getInstance().free(diskIds[i]);
        disks.RemoveAt(i);
        diskIds.RemoveAt(i);
    }
