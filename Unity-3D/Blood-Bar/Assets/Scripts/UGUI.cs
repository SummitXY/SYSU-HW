...
public class UGUI : MonoBehaviour {
    ...
    void Update()
    {
        ...
        rectBloodPos.anchoredPosition = new Vector2(vec2.x - Screen.width / 2 + 0, vec2.y - Screen.height / 2 + 20);
    }

    private void OnGUI()
    {
        if (GUI.Button(new Rect(620, 160, 37, 37), "+"))
        {
            reduceBlood -= 10;
            if(reduceBlood < 0)
            {
        ...
        if (GUI.Button(new Rect(325, 160, 37, 37), "-"))
        {
            reduceBlood += 10;
            if(reduceBlood > 200)
            {
